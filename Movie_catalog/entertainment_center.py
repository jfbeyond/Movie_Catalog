...
# Initialize the movie array that will contain the movies' data
c = len(my_favorite)
movies = [None]*c

# Populate movies array with the data retrieved from API
for s in range(0, c):
    # Call get_movie_id function to retrieve movie id
    fav_id = media.Findmovie(my_favorite[s]).get_movie_id()
    # Retrieve movie data with get_movie_data() with its id
